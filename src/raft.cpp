...
void Raft::generate(SliceDataStorage& storage)
{
    ...
    const ExtruderTrain& adhesion_train = settings.get<ExtruderTrain&>("adhesion_extruder_nr");
    const coord_t distance = adhesion_train.settings.get<coord_t>("raft_margin");
    ...
    const coord_t shield_line_width_layer0 = adhesion_train.settings.get<coord_t>("skirt_brim_line_width");
    ...
    const coord_t smoothing = adhesion_train.settings.get<coord_t>("raft_smoothing");
...
coord_t Raft::getTotalThickness()
{
    ...
    const ExtruderTrain& adhesion_train = mesh_group_settings.get<ExtruderTrain&>("adhesion_extruder_nr");
    return adhesion_train.settings.get<coord_t>("raft_base_thickness")
        + adhesion_train.settings.get<coord_t>("raft_interface_thickness")
        + adhesion_train.settings.get<size_t>("raft_surface_layers") * adhesion_train.settings.get<coord_t>("raft_surface_thickness");
}
...
coord_t Raft::getZdiffBetweenRaftAndLayer1()
{
    ...
    const coord_t airgap = std::max<unsigned int>(coord_t(0), adhesion_train.settings.get<coord_t>("raft_airgap"));
    const coord_t layer_0_overlap = adhesion_train.settings.get<coord_t>("layer_0_z_overlap");
    const coord_t layer_height_0 = adhesion_train.settings.get<coord_t>("layer_height_0");
    const coord_t z_diff_raft_to_bottom_of_layer_1 = std::max<unsigned int>(coord_t(0), airgap + layer_height_0 - layer_0_overlap);
...
size_t Raft::getTotalExtraLayers()
{
    const Settings& mesh_group_settings = Application::getInstance().current_slice->scene.current_mesh_group->settings;
    const ExtruderTrain& adhesion_train = mesh_group_settings.get<ExtruderTrain&>("adhesion_extruder_nr");
    if (adhesion_train.settings.get<EPlatformAdhesion>("adhesion_type") != EPlatformAdhesion::RAFT)
    {
        return 0;
    }
    return 2 + adhesion_train.settings.get<size_t>("raft_surface_layers") + getFillerLayerCount();
}
