...
void FffGcodeWriter::writeGCode(SliceDataStorage& storage, TimeKeeper& time_keeper)
{
    ...
    const std::function<void (LayerPlan*)>& consume_item =
        ...
        {
            Progress::messageProgress(Progress::Stage::EXPORT, std::max<unsigned int>(0, gcode_layer->getLayerNr()) + 1, total_layers);
    ...
    max_object_height = std::max<unsigned int>(max_object_height, storage.model_max.z);
...
void FffGcodeWriter::processStartingCode(const SliceDataStorage& storage, const size_t start_extruder_nr)
{
    ...
    gcode.startExtruder(start_extruder_nr, false,retraction_config);
...
bool FffGcodeWriter::processMultiLayerInfill(const SliceDataStorage& storage, LayerPlan& gcode_layer, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SliceLayerPart& part) const
{
    ...
    {
        const size_t combined_infill_layers = std::max<u_int>(unsigned(1), round_divide(mesh.settings.get<coord_t>("infill_sparse_thickness"), std::max<u_int>(mesh.settings.get<coord_t>("layer_height"), coord_t(1))));
                ...
                if (infill_below_skin.offset(-(infill_skin_overlap + tiny_infill_offset)).size())
                {
                    ...
                    if (infill_not_below_skin.offset(-(infill_skin_overlap + tiny_infill_offset)).size())
                    {
                        ...
                        Infill infill_comp(pattern, zig_zaggify_infill, connect_polygons, infill_below_skin, /*outline_offset =*/ 0
                            , infill_line_width, infill_line_distance_here, infill_overlap, infill_multiplier, infill_angle, gcode_layer.z, infill_shift, std::max<u_int>(1, (int)wall_line_count), infill_origin
                            , perimeter_gaps
                            , connected_zigzags
                            , use_endpieces
                            , skip_some_zags
...
{
    bool added_something = false;
    const SupportLayer& support_layer = storage.support.supportLayers[std::max<u_int>(0, gcode_layer.getLayerNr())]; // account for negative layer numbers for raft filler layers
